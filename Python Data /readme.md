...
1. Finds the **latest** CSV/XLSX (by date in filename or file modified time).
2. Cleans and standardizes columns (renames, trimming, datatype enforcement, date parsing).
3. Deduplicates rows based on dataset-specific keys.
4. Writes a single **latest snapshot parquet** per dataset into the “Second Staging Area”.
5. Builds a final consolidated view with:
...
If using `.xls` inputs, your environment may also require `xlrd`.
